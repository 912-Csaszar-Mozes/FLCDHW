...
def test(nr_runs=1, test_size=1000):
    for i in range(nr_runs):
        ...
        with open("test_out/test_str" + str(i + 1) + ".txt", "w") as f:
            ht = SymbolTable()
            f.write("Initializing string key test number " + str(i + 1) + " with " + str(
                test_size) + " value inserts into the hashtable\n")
            letters = string.ascii_lowercase
            keys = [''.join(random.choice(letters) for i in range(random.randint(7, 10))) for _ in range(test_size)]
            values = [random.randint(1000, 150000) for _ in range(test_size)]
            for j in range(len(keys)):
                ht.add(keys[j], values[j])
            f.write("Added " + str(test_size) + " values successfully.\n")
